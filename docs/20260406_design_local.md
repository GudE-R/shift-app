...
CREATE TABLE staff (
    ...
    max_consecutive_days INT,    -- 連続勤務上限（未設定時はデフォルト5日で警告）
    memo TEXT,                   -- 暗黙ルール等
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
```
...
| 項目 | データソース | 算出方法 |
|------|-------------|----------|
| 今週のシフトサマリー | shift_entries | 当週の日別シフト数・合計労働時間を集計 |
| 充足率 | shift_entries × store_requirements | 時間帯別の配置人数 ÷ 必要人数 × 100（%） |
| 警告数 | Local Validator の結果 | 休憩不足・長時間労働・連続勤務超過の件数 |

### 7.2 更新タイミング
- ダッシュボード表示時にローカルDBから再計算（キャッシュなし・常に最新）。

## 8. 出力設計

### 8.1 PDF出力
- **レイアウト**: A3横。スタッフ名を左列、日付を横軸としたグリッド形式。
- **ライブラリ**: jspdf + jspdf-autotable（ローカル完結、サーバー不要）。
- **内容**: 店舗名・対象期間・各スタッフのシフト時間・ポジション色分け。

### 8.2 CSV出力
- **形式**: UTF-8 BOM付き（Excel対応）。
- **カラム**: 日付, スタッフ名, 店舗名, ポジション, 開始時間, 終了時間, 休憩(分)。
- **対象範囲**: シフトカレンダーで表示中の期間。
